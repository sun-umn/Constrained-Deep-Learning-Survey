...
if __name__ == '__main__':
    ...
    for i in range(START_REPEAT, END_REPEAT):  # Example for 3 initializations
        ...
        if MODEL_TYPE == 'MLP':
            ...
            torch.save(
                model.state_dict(), os.path.join(MODEL_DIR, f'model_pytorch_{i}.pt')
            )
            ...
            #      do_constant_folding=True,  # whether to execute constant folding for optimization  # noqa
            #      input_names=['input'],  # name your input
            #      output_names=['output'] # name your output
            #      #dynamic_axes={'input': {0: 'batch_size'},  # if your model supports dynamic batch size  # noqa
            #      #              'output': {0: 'batch_size'}})
            # )
            ...
            #    node.name = node.name.replace('::', '__')  # Replace problematic characters  # noqa
            ...
            keras_model = create_tf_model(
                1, NUM_FEATURES, NUM_LAYERS, LAYER_WIDTH, MODEL_OUT_ACTIVATION
            )
            # keras_model.summary()
            ...
            for i, layer in enumerate(model.layers):
                ...
                if isinstance(layer, nn.Linear):
                    ...
                    keras_model.layers[i - k].set_weights(
                        [weight.T, bias]
                    )  # Multiply by 3 because of batch norm and activation layers
                elif isinstance(layer, nn.BatchNorm1d):
                    # print(i-k)
                    # print(keras_model.layers[i-k].name)
                    gamma, beta = layer.weight.data.numpy(), layer.bias.data.numpy()
                    mean, var = (
                        layer.running_mean.data.numpy(),
                        layer.running_var.data.numpy(),
                    )
            ...
            pytorch_output = (
                model(torch.from_numpy(input_data).float()).detach().numpy()
            )
