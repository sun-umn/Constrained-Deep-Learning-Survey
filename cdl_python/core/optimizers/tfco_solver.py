...
class problem(tfco.ConstrainedMinimizationProblem):
    ...
    def objective(self):
        ...
        f = (
            self._obj_loss(pred0, self._train_y_0)
            + self._obj_loss(pred1, self._train_y_1)
        ) / (num0 + num1)
        return f
    ...
    def constraints(self):
        ...
        num1 = self._train_X_1.shape[0]
        diff = (
            self._constr_loss(pred0, self._train_y_0) / num0
    ...
    def evaluation(self):
        ...
        accuracy = (
            (
                tf.reduce_sum(tf.cast(tf.equal(pred0, train_y_0), tf.float32))
                + tf.reduce_sum(tf.cast(tf.equal(pred1, train_y_1), tf.float32))
            )
            / (num0 + num1)
        ).numpy()
        ...
        ft = (
            (
                self._obj_loss(pred0, self._test_y_0)
                + self._obj_loss(pred1, self._test_y_1)
            )
            / (num0 + num1)
        ).numpy()
        difft = np.abs(
            (
                self._constr_loss(pred0, self._test_y_0) / num0
                - self._constr_loss(pred1, self._test_y_1) / num1
            ).numpy()
        )
        # print(tf.multiply(pos_s, self._alpha-1))
        ...
        accuracyt = (
            (
                tf.reduce_sum(tf.cast(tf.equal(pred0, test_y_0), tf.float32))
                + tf.reduce_sum(tf.cast(tf.equal(pred1, test_y_1), tf.float32))
            )
            / (num0 + num1)
        ).numpy()

        return f, [cons0, cons1], diff, accuracy, ft, [cons0t, cons1t], difft, accuracyt
...
class TFCO_problem:
    def __init__(self, cfg, data, device, model, fn=''):  # dataset is in
        ...
        if constr_loss == 'BCE':
            constr_loss = tf.keras.losses.BinaryCrossentropy(
                reduction=tf.keras.losses.Reduction.SUM
            )
    ...
    def train(self):
        ...
        loss_fn, update_ops_fn, multipliers = tfco.create_lagrangian_loss(self.problem)

        optimizer = tf.keras.optimizers.AdamW()  # Adagrad() #learning_rate=0.01
        var_list = (
            list(self.problem.trainable_variables)
